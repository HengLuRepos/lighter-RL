...
class Actor(nn.Module):
    def __init__(self, env):
        ...
        self.quant_state = taq.QuantStub()
        self.dequant_output = taq.DeQuantStub()
    def forward(self, x):
        x = self.quant_state(x)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
...
"""
    ...
    actor.load_model(f"models/ddpg-{args.env_id}-seed-{args.seed}-actor.pt")
    ...
    target_actor = taq.prepare_qat(target_actor.to(device).train(), inplace=True)
    actor.train()
    qf1.train()
    ...
    target_actor_int8 = taq.convert(target_actor.eval().to('cpu'), inplace=False)
    actor_int8.save_model(f"models/qat/ddpg-{args.env_id}-seed-{args.seed}-actor-x86.pt")
    all_state_dict = {
        'actor': actor.state_dict(),
        'q1': qf1.state_dict(),
        'q1_target': qf1_target.state_dict(),
        'actor_target': target_actor.state_dict()
    }
    actor.save_model(f"models/qat/ddpg-{args.env_id}-seed-{args.seed}-actor.pt")
    torch.save(all_state_dict, f"models/qat/ddpg-{args.env_id}-seed-{args.seed}.pt")
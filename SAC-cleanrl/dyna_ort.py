...
def make_env(env_id, seed, idx, capture_video, run_name):
    def thunk():
        if capture_video and idx == 0:
            ...
            env = gym.wrappers.RecordVideo(env, f"videos/SAC-{env_id}-dynamic")
        else:
            env = gym.make(env_id)
...
if __name__ == "__main__":
    ...
    quantized_model = quantize_dynamic(f"models/onnxQuant/SAC-{args.env_id}-prep.onnx", f"models/onnxQuant/SAC-{args.env_id}-dyna.onnx")
    session = ort.InferenceSession(f"models/onnxQuant/SAC-{args.env_id}-dyna.onnx", providers=ort.get_available_providers())

...
def make_env(env_id, idx, capture_video, run_name, gamma):
    def thunk():
        if capture_video:
            ...
            env = gym.wrappers.RecordVideo(env, f"videos/PPO-{env_id}-baseline-ort")
        else:
            env = gym.make(env_id)
...
if __name__ == "__main__":
    ...
    fp32_return = []
    fp32_ram = []
    session = ort.InferenceSession(f"models/PPO-{args.env_id}.onnx", providers=ort.get_available_providers())
    input_name = session.get_inputs()[0].name
    states, _ = envs.reset(seed=2 + 100)
    for i in range(10):

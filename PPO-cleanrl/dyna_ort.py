...
def make_env(env_id, idx, capture_video, run_name, gamma):
    def thunk():
        if capture_video:
            ...
            env = gym.wrappers.RecordVideo(env, f"videos/PPO-{env_id}-dynamic")
        else:
            env = gym.make(env_id)
...
if __name__ == "__main__":
    ...
    quantized_model = quantize_dynamic(f"models/onnxQuant/PPO-{args.env_id}-prep.onnx", f"models/onnxQuant/PPO-{args.env_id}-dyna.onnx")
    session = ort.InferenceSession(f"models/onnxQuant/PPO-{args.env_id}-dyna.onnx", providers=ort.get_available_providers())

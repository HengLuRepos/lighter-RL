...
for i in range(len(seed)):
    ...
    quant_end = time.time()
    td3_int8.save_model(f"models/ddpg-quantize-{config.env_name}.pt")
...
print(f"fp32 avg inference time: {np.mean(fp32_time):.2f} +/- {np.std(fp32_time):.2f}")
print(f"fp32 avg return: {np.mean(fp32_return):.2f} +/- {np.std(fp32_return):.2f}")
print(f"fp32 avg ep length: {np.mean(fp32_step):.2f} +/- {np.std(fp32_step):.2f}")

print(f"int8 avg inference time: {np.mean(int8_time):.2f} +/- {np.std(int8_time):.2f}")
print(f"int8 avg return: {np.mean(int8_return):.2f} +/- {np.std(int8_return):.2f}")
print(f"int8 avg ep length: {np.mean(int8_step):.2f} +/- {np.std(int8_step):.2f}")

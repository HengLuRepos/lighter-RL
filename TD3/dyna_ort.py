...
quantized_model = quantize_dynamic(f"models/onnxQuant/TD3-{config.env}-prep.onnx", f"models/onnxQuant/TD3-{config.env}-dyna.onnx")
session = ort.InferenceSession(f"models/onnxQuant/TD3-{config.env}-dyna.onnx", providers=ort.get_available_providers())
input_name = session.get_inputs()[0].name
state, info = env.reset(seed=seed[0]+100)
with torch.no_grad():
    for i in range(1):
        origin_start = time.time()
        env = gym.wrappers.RecordVideo(env=env, video_folder="video",name_prefix=f"TD3-{config.env}-dynamic")

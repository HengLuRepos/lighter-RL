...
file = f"{args.algs}/csv/onnxQuant/{args.env_id}.csv"
...
ram, ram_std = data[6].values, data[7].values
...
plt.savefig(f"{args.algs}/figs/onnxQuant/{args.algs}-{args.env_id}-avg-return.png")

plt.figure()
plt.bar(methods, infer, yerr=infer_std)
plt.ylabel("inference time")
plt.title(f"Quantization {args.algs} {args.env_id} inference time")
plt.savefig(f"{args.algs}/figs/onnxQuant/{args.algs}-{args.env_id}-infer-time.png")
...
plt.savefig(f"{args.algs}/figs/onnxQuant/{args.algs}-{args.env_id}-length.png")

plt.figure()
plt.bar(methods, ram, yerr=ram_std)
plt.ylabel("ram usage")
plt.title(f"Quantization {args.algs} {args.env_id} ram usage")
plt.savefig(f"{args.algs}/figs/onnxQuant/{args.algs}-{args.env_id}-ram.png")

plt.figure()
plt.bar(methods, energy)
plt.ylabel("energy")
plt.title(f"Quantization {args.algs} {args.env_id} energy")
plt.savefig(f"{args.algs}/figs/onnxQuant/{args.algs}-{args.env_id}-energy.png")

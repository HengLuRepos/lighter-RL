...
if __name__ == "__main__":
    ...
    origin_session = ort.InferenceSession(f"models/PPO-{args.env_id}.onnx", providers=ort.get_available_providers())
    all_steps = 0
    states = []
    ...
    while all_steps < 10000:
        ...
        action = origin_session.run(None, {input_name: state.astype(np.float32)})[0]
    ...
    static_opts = {"ActivationSymmetric":False,
                 "WeightSymmetric":True}
    
    quantized_model = quantize_static(model_input=f"models/onnxQuant/PPO-{args.env_id}-prep.onnx", 
                                      model_output=f"models/onnxQuant/PPO-{args.env_id}-static.onnx",
                                      calibration_data_reader=qdr,
                                      extra_options=static_opts)
    session = ort.InferenceSession(f"models/onnxQuant/PPO-{args.env_id}-static.onnx", providers=ort.get_available_providers())

...
session = ort.InferenceSession(f"models/pruning/TD3-{config.env_name}-{args.prune_amount}-l{args.n}.onnx", providers=ort.get_available_providers())
input_name = session.get_inputs()[0].name
env.reset(seed=eval_seed[0]+100)
for i in range(1):
    origin_start = time.time()
    env = gym.wrappers.RecordVideo(env=env, video_folder="video",name_prefix=f"TD3-{config.env}-pruning")
